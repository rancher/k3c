...
func (c *criServiceManager) GetResolver(ctx context.Context, auth *runtime.AuthConfig, tracker docker.StatusTracker) (remotes.Resolver, error) {
	svc, err := c.namespaceRequired(ctx)
	if err != nil {
		return nil, errors.Wrap(err, "Failed to get resolver")
	}
	return docker.NewResolver(docker.ResolverOptions{
		Hosts:   svc.registryHosts(auth),
		Tracker: tracker,
	}), nil
